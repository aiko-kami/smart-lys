...
const menu = [
	...
	{ label: "Tâches", path: "/tasks", icon: <FaListCheck className="shrink-0" /> },
...
export default function Sidebar({ onClose }: SidebarProps) {
	...
	return (
		<aside className="flex h-full flex-col relative">
			...
			<nav className="flex-1 space-y-1 overflow-y-auto p-3">
				{menu.map((item) => (
					<Link key={item.path} href={item.path} prefetch={false} onClick={onClose}>
						<span className="flex w-full items-center gap-4 rounded-xl px-4 py-3 text-sm text-gray-300 transition hover:bg-blue-500/10 hover:text-blue-400">
							{item.icon}
							{item.label}
						</span>
					</Link>
				...
				<button onClick={refresh} className="w-full cursor-pointer">
					<span className="flex w-full items-center gap-4 rounded-xl px-4 py-3 text-sm text-gray-300 transition hover:bg-emerald-500/10 hover:text-emerald-400">
			...
			<div className="border-t border-white/10 px-5 py-3 text-xs text-gray-500 text-right">v1.0 Smart Lys</div>
		</aside>
	);
}
